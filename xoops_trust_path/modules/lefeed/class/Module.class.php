...
define('LEFEED_FRAME_PERFORM_SUCCESS', 1);
define('LEFEED_FRAME_PERFORM_FAIL', 2);
define('LEFEED_FRAME_INIT_SUCCESS', 3);

define('LEFEED_FRAME_VIEW_NONE','none');
define('LEFEED_FRAME_VIEW_SUCCESS','success');
define('LEFEED_FRAME_VIEW_ERROR','error');
define('LEFEED_FRAME_VIEW_INDEX','index');
define('LEFEED_FRAME_VIEW_INPUT','input');
define('LEFEED_FRAME_VIEW_PREVIEW','preview');
define('LEFEED_FRAME_VIEW_CANCEL','cancel');

/**
 * Lefeed_Module
**/
class Lefeed_Module extends Legacy_ModuleAdapter
{
	public /*** string ***/ $mActionName = null;

	public /*** Lefeed_AbstractAction ***/ $mAction = null;

	public /*** bool ***/ $mAdminFlag = false;

	public /*** Lefeed_AssetManager ***/ $mAssetManager = null;
	...
	protected /*** Enum[] ***/ $_mAllowViewNames = array(
		LEFEED_FRAME_VIEW_NONE,
		LEFEED_FRAME_VIEW_SUCCESS,
		LEFEED_FRAME_VIEW_ERROR,
		LEFEED_FRAME_VIEW_INDEX,
		LEFEED_FRAME_VIEW_INPUT,
		LEFEED_FRAME_VIEW_PREVIEW,
		LEFEED_FRAME_VIEW_CANCEL
	);
	...
	**/
	public function startup()
	{
		...
		XCube_DelegateUtils::call('Module.lefeed.Global.Event.GetAssetManager',new XCube_Ref($this->mAssetManager),$this->mXoopsModule->get('dirname'));
	...
	**/
	private function _getDefaultActionName()
	{
		...
		$dataname = isset($dataname) ? $dataname : 'Entry';
	...
	**/
	public function getRenderSystemName()
	{
		...
		if(!defined('LEFEED_ADMIN_RENDER_REGISTED'))
		{
			define('LEFEED_ADMIN_RENDER_REGISTED',true);
			$root =& XCube_Root::getSingleton();
			$root->overrideSiteConfig(
				array(
					'RenderSystems' => array(
						'Lefeed_AdminRenderSystem' => 'Lefeed_AdminRenderSystem'
					),
					'Lefeed_AdminRenderSystem' => array(
						'root' => LEFEED_TRUST_PATH,
						'path' => '/admin/class/LefeedAdminRenderSystem.class.php',
						'class' => 'Lefeed_AdminRenderSystem'
					)
				)
			);
		}
	
		return 'Lefeed_AdminRenderSystem';
	}
	...
	private function _createAction()
	{
		...
		switch(true)
		{
			case file_exists(
				...
				break;
			case file_exists(
				$path = LEFEED_TRUST_PATH . '/' . $fileName
			):
				break;
			default:
				return false;
		...
		$className = 'Lefeed_' . ($this->mAdminFlag ? 'Admin_' : '')
			. ucfirst($this->mActionName) . 'Action';
		if(class_exists($className))
		{
			$this->mAction = new $className();
		}
		if(!$this->mAction instanceof Lefeed_AbstractAction)
		{
			return false;
		}
	...
	**/
	...
	{
		/**
		 * Module.lefeed.Global.Event.Exception.ActionNotFound
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.lefeed.Global.Event.Exception.ActionNotFound',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.lefeed.Global.Event.Exception.Preparation
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.lefeed.Global.Event.Exception.Preparation',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.lefeed.Global.Event.Exception.Permission
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.lefeed.Global.Event.Exception.Permission',$this->mAssetManager->mDirname);
		/**
		...
		**/
		...
		$root =& XCube_Root::getSingleton();
		$root->mController->executeRedirect(XOOPS_URL, 1, _MD_LEFEED_ERROR_PERMISSION);
	}
}
